...
const History = () => {
  ...
  const handleCardIdChange = async (e) => {
    ...
    if (e.target.value !== '') {
      try {
        const response = await fetch(`http://localhost:3000/api/users/search?query=${e.target.value}`);
        const data = await response.json();
          ...
          <TabContext value={value}>
            ...
            <TabPanel className='px-[0!important]' value="3">
              ...
              <div className='max-w-lg m-auto'>
                {/* handleAddUser */}
                <form onSubmit={(e) => handleAddUser(e)} className='flex flex-col justify-center items-center gap-2' action="http://localhost:3000/api/auth/register" method="post">
                  <div className='border rounded-xl w-full overflow-hidden flex justify-center items-center'>
